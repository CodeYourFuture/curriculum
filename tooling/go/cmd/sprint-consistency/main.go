...
func main() {
	...
	flag.StringVar(&rootExcludes, "exclude-roots", "", "Directories in the root to ignore (comma-separated)")
	flag.Parse()

	excudedDirectories := computeExcludedDirectories(rootDirectory, rootExcludes)
...
func computeExcludedDirectories(rootDirectory, rootExcludes string) map[string]bool {
	if rootExcludes == "" {
		return nil
	}
	excludedDirectories := make(map[string]bool)
	for _, rootExclude := range strings.Split(rootExcludes, ",") {
		excludedDirectories[filepath.Join(rootDirectory, rootExclude)] = true
	}
