...
You may have heard of, or used, generative AI tools (shortened to AI or AI tools from now on) like ChatGPT, Copilot or Gemini. These are powerful tools that we have seen both disrupt and assist the learning for trainees at {{<our-name>}}. This page aims to guide you in best practices for AI use in the ITD.
...
At this stage, you may be taking your first step in your tech career. AI can help you orientate yourself, explaining unfamiliar concepts and answering basic questions. It is acceptable to explore AI tools as part of understanding the digital landscape. Let's see how it could affect our learning journey.

### AI: A predictive word-generator

AI assistants, like those mentioned above, are often large language models (LLMs). LLMs are "trained", on huge amounts of data, to predict what word should come next in a sentence. Because they can read and generate text data, it feels like you're messaging a smart "entity" but it is just a tool that has an algorithm for guessing what words should follow your input ("prompt").

Because AI tools "guess" the text to generate, they can make silly mistakes or make up things that don’t exist (_hallucinations_). For instance, a famous hallucination from 2024 was that many AI tools said there were two 'r's in the word strawberry.
...
At {{<our-name>}}, we prioritise building understanding so trainees can be employable in the tech industry. Employability in software engineering involves knowing how to debug broken code, find information you might not already know, and breakdown problems to find a solution. {{<our-name>}} mentors you encounter during the ITD and beyond will try to help you without finding the answers or writing the code for you - all so that you learn how to overcome hurdles yourself, and apply that skill in the workplace when mentors aren't around.
...
AI can make mistakes when it's guessing what output to return to the user, but it can also be led to give incorrect information when a prompt isn't accurate. Which means we need to ask good questions (or {{<tooltip name="prompts">}} The input we give an AI assistant. {{</tooltip>}}). As learners early in the {{<our-name>}} journey it might be harder to ask accurate questions to AI chatbots, which means it is more likely AI will give misleading results. It's something to be aware and cautious of - we will discuss this more in later courses.
...
A {{<our-name>}} trainee is assigned coursework on a {{<our-name>}} course. The assignment is to build a homepage for a library which displays their twenty newest books. It requires that they use CSS Grid (a CSS feature used to display HTML elements in a grid format). They think that ChatGPT could help them complete this task.
...
A {{<our-name>}} trainee balances fulltime work and family responsibilities with studying at {{<our-name>}}. They are struggling to keep up with the course. In particular, they are struggling to understand some fundamental concepts.
