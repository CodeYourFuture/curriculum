...
When we want to do something with a remote through terminal we will use the `git remote` command. First we need to `add` the remote to our repository. Copy the url for your repository from GitHub and use it with teh command below:
...
We can create as many remotes as we like so long as they have unique names. By convention we will keep using `origin` as the name for our remote source control repo. We can see a list of all available remotes by typing `got remote` with no other arguments.
